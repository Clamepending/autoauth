...
- `SLACK_WEBHOOK_URL` to post every submitted order to an admin Slack channel
- `OTTOAUTH_ADMIN_SMS_TO=+16095551212` plus `TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN`, and `TWILIO_FROM_NUMBER` or `TWILIO_MESSAGING_SERVICE_SID` to text operators for every submitted order
...
3. **Configure environment variables** in the Vercel project (Settings -> Environment Variables):
   ...
   - **Optional:**
     ...
     - `SLACK_WEBHOOK_URL` - Slack [Incoming Webhook](https://api.slack.com/messaging/webhooks) URL. Submitted orders and legacy agent requests are posted here. Set to different values per environment in Vercel (Production vs Preview) if you want different channels.
     - `OTTOAUTH_ADMIN_SMS_TO`, `TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN`, and `TWILIO_FROM_NUMBER` or `TWILIO_MESSAGING_SERVICE_SID` - Twilio SMS alerts for every submitted order. Use a sender that is verified/registered for the recipient's country; Twilio API acceptance is not carrier delivery.
