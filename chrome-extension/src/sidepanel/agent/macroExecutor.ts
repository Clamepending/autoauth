...
export async function executeMacro(
  ...
  try {
    for (let i = 0; i < macro.steps.length; i++) {
      ...
      try {
        const stepResult = await executeStepDirectly(step.tool, resolvedInput, apiKey);
    ...
    if (!lastScreenshot) {
      try {
        const screenshotResult = await executeStepDirectly('computer', { action: 'screenshot', tabId }, apiKey);
...
async function executeStepDirectly(
  ...
  return execTool(tool, input, apiKey);
}
