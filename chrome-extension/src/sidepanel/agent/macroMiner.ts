...
async function getBackendConfig(): Promise<BackendConfig | null> {
  return new Promise((resolve) => {
    chrome.storage.local.get(
      ...
      (result) => {
        const url = (result[STORAGE_KEY_OTTOAUTH_URL] as string | undefined)?.replace(/\/+$/, '');
...
function backendHeaders(backend: BackendConfig, json = false): Record<string, string> {
  const headers: Record<string, string> = {};
  if (json) headers['Content-Type'] = 'application/json';
  if (backend.deviceId) headers['X-OttoAuth-Device-Id'] = backend.deviceId;
...
export async function maybeRunMiningPass(domain: string, apiKey: string): Promise<void> {
  ...
  if (backend) {
    try {
      const res = await fetch(`${backend.url}/api/macros/mine`, {
        method: 'POST',
        headers: backendHeaders(backend, true),
        body: JSON.stringify({ domain, deviceId: backend.deviceId }),
      });
      if (res.ok) {
...
async function fetchRemoteMacros(): Promise<Macro[]> {
  ...
  for (const domain of activeDomains.slice(0, 5)) {
    try {
      const res = await fetch(`${backend.url}/api/macros/${encodeURIComponent(domain)}`, {
...
export async function reportMacroOutcome(macroId: string, success: boolean): Promise<void> {
  ...
  try {
    await fetch(`${backend.url}/api/macros/outcome`, {
      method: 'POST',
      headers: backendHeaders(backend, true),
      body: JSON.stringify({ macroId, success, deviceId: backend.deviceId }),
    });
  } catch (e) {
