...
export async function finalizeTrace(success: boolean): Promise<string | null> {
  ...
  try {
    await uploadTraceToBackend(domain, trace);
  } catch (e) {
...
async function getBackendConfig(): Promise<{ url: string; deviceId?: string; token?: string } | null> {
  return new Promise((resolve) => {
...
async function uploadTraceToBackend(domain: string, trace: TaskTrace): Promise<void> {
  ...
  const res = await fetch(`${backend.url}/api/macros/traces`, {
    method: 'POST',
    headers,
    body: JSON.stringify({ domain, trace, deviceId: backend.deviceId }),
  });
