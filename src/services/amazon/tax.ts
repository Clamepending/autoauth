/**
 * Estimate US sales tax from a shipping address string.
 *
 * Uses average combined (state + local) tax rates per state.
 * These are approximations — actual tax depends on exact locality,
 * product category, and Amazon-specific tax rules. The rates here
 * are intentionally rounded up slightly to reduce under-charging.
 */

const STATE_TAX_RATES: Record<string, number> = {
  AL: 0.0925, AR: 0.0945, AZ: 0.084, CA: 0.087, CO: 0.078,
  CT: 0.0635, DC: 0.06, DE: 0, FL: 0.07, GA: 0.074,
  HI: 0.0444, ID: 0.06, IL: 0.0885, IN: 0.07, IA: 0.07,
  KS: 0.087, KY: 0.06, LA: 0.0955, ME: 0.055, MD: 0.06,
  MA: 0.0625, MI: 0.06, MN: 0.075, MS: 0.0707, MO: 0.083,
  MT: 0, NE: 0.07, NV: 0.082, NH: 0, NJ: 0.066,
  NM: 0.078, NY: 0.085, NC: 0.07, ND: 0.07, OH: 0.0724,
  OK: 0.09, OR: 0, PA: 0.0634, RI: 0.07, SC: 0.074,
  SD: 0.064, TN: 0.0955, TX: 0.082, UT: 0.072, VT: 0.063,
  VA: 0.058, WA: 0.1025, WV: 0.066, WI: 0.055, WY: 0.054,
  AK: 0.018, PR: 0.115, GU: 0.04, VI: 0.05,
};
...
const ZIP_PREFIX_TO_STATE: Record<string, string> = {
  ...
  "850":"AZ","852":"AZ","853":"AZ","855":"AZ","856":"AZ","857":"AZ","859":"AZ","860":"AZ",
...
function extractState(address: string): string | null {
  ...
  // Try "State ZIPCODE" pattern (e.g. "NY 10001" or "New York, NY 10001")
  const stateZipMatch = upper.match(/\b([A-Z]{2})\s+(\d{5})\b/);
  if (stateZipMatch) {
    const abbr = stateZipMatch[1];
    if (abbr in STATE_TAX_RATES) return abbr;
  }

  // Try zip code alone and map it to state
  ...
  // Try standalone 2-letter state abbreviation
...
export function estimateTax(
  ...
  const state = extractState(shippingAddress);
  const rate = state ? (STATE_TAX_RATES[state] ?? DEFAULT_TAX_RATE) : DEFAULT_TAX_RATE;
  const taxCents = Math.ceil(itemPriceCents * rate);
  const totalCents = itemPriceCents + taxCents;
